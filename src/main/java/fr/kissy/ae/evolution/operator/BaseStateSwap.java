 ...
 * @version $Id$
 */
public class BaseStateSwap implements EvolutionaryOperator<BaseState> {
    private final int possibleActionsSize;
    private final List<Action> possibleActions;
    private final NumberGenerator<Probability> mutationProbability;

    public BaseStateSwap(List<Action> possibleActions, NumberGenerator<Probability> mutationProbability) {
    ...
    private BaseState mutate(BaseState baseState, Random rng) {
        ...
        int randomPoint2 = (int) (Math.random() * actionsSize);
        Action swap = copy.getActions().get(randomPoint);
        copy.getActions().set(randomPoint, copy.getActions().get(randomPoint2));
        copy.getActions().set(randomPoint2, swap);
        return copy;
    }
}
