 ...
 * @version $Id$
 */
public class BaseStateDeletion implements EvolutionaryOperator<BaseState> {
    private final int possibleActionsSize;
    private final List<Action> possibleActions;
    private final NumberGenerator<Probability> mutationProbability;

    public BaseStateDeletion(List<Action> possibleActions, NumberGenerator<Probability> mutationProbability) {
    ...
    private BaseState mutate(BaseState baseState, Random rng) {
        ...
        for (int j = (int) (Math.random() * actionsSize); j < actionsSize - 1; j++) {
            copy.getActions().set(j, copy.getActions().get(j + 1));
        }
        return copy;
    }
}
